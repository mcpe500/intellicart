...
app.post('/api/products/:id/reviews', async (c) => {
  ...
  (c as any).set('user', { userId: 1 });
  return ProductController.addReviewToProduct(c);
});
...
describe('Products API Tests', () => {
  ...
  it('should retrieve all products successfully', async () => {
    ...
    const createdProduct = responseBody.find((p: any) => p.id === testProductId);
  ...
  it('should add a review to the product successfully', async () => {
    ...
    const response = await app.request(`/api/products/${testProductId}/reviews`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${userToken}`
