...
export class BaseController<T> {
  ...
  async getAll(c: Context) {
    try {
      ...
      return this.handleError(
        error,
        c,
        `Error retrieving all ${this.tableName}`,
      );
    }
  }
  ...
  async getById(c: Context) {
    try {
      ...
      if (!item) {
        throw new NotFoundError(`${this.tableName.slice(0, -1)} not found`, {
          id,
      ...
      return this.handleError(
        error,
        c,
        `Error retrieving ${this.tableName.slice(0, -1)} by ID`,
      );
    }
  }
  ...
  async create(c: Context, createData: Partial<T>) {
    try {
      ...
      const createdItem = await this.db.create(this.tableName, newItem as T);
      ...
      return this.handleError(
        error,
        c,
        `Error creating ${this.tableName.slice(0, -1)}`,
      );
    }
  }
  ...
  async update(c: Context, updateData: Partial<T>) {
    try {
      ...
      return this.handleError(
        error,
        c,
        `Error updating ${this.tableName.slice(0, -1)}`,
      );
    }
  }
  ...
  async delete(c: Context) {
    try {
      ...
      return this.handleError(
        error,
        c,
        `Error deleting ${this.tableName.slice(0, -1)}`,
      );
    }
  }
}
