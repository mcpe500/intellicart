...
async function seedDatabase() {
  try {
    ...
    // Clear existing data by finding all records and deleting them individually
    const existingProducts = await db.findAll('products');
    for (const product of existingProducts) {
      await db.delete('products', (product as any).id);
    }
    
    const existingUsers = await db.findAll('users');
    for (const user of existingUsers) {
      await db.delete('users', (user as any).id);
    }
    
    const existingOrders = await db.findAll('orders');
    for (const order of existingOrders) {
      await db.delete('orders', (order as any).id);
    }
    ...
    await db.create('users', sellerUser);
    await db.create('users', buyerUser);
